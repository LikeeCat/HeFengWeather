...
- (void)setDynamic:(DiscoverTable *)dynamic
{
    ...
    if (_dynamic.type.value == 0) { //纯文字
        NSMutableAttributedString *content = [Utility emotionStrWithString:_dynamic.text.value];
    ...
    } else if (_dynamic.type.value == 1) { //带图片
        CGFloat contentHeight = 0;
        if (_dynamic.text.value.length <= 0) {//不存在文字，肯定是纯图片
        ...
        for (int i = 0; i < _dynamic.imageCount.value; i++) {
            if (i > 8) {
                break;
            }
            int rowNum = i/3;
            ...
            if (_dynamic.imageCount.value == 1) {
...
//            imageView.image = [UIImage imageNamed:imageName];
            ...
            imageView.image = [UIImage imageWithData:dynamic.image.value];
        }
        
    } else if (_dynamic.type.value == 2) { //小视频
        CGFloat contentHeight = 0;
        if (_dynamic.text.value.length <= 0) {//不存在文字，肯定是纯图片
            _contentLabel.frame = CGRectZero;
            _contentLabel.hidden = YES;
...
+ (CGFloat)getHeightWithModel:(DiscoverTable *)dynamic
{
    if (dynamic.type.value == 0) { //纯文字
        NSMutableAttributedString *content = [Utility emotionStrWithString:dynamic.text.value];
        [content addAttribute:NSFontAttributeName value:kContentFont range:NSMakeRange(0, content.length)];
        CGSize maxSize = CGSizeMake(kDynamicWidth, MAXFLOAT);
        CGSize attrStrSize = [content boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin context:nil].size;
        return attrStrSize.height;
    } else if (dynamic.type.value == 1) { //附带图片
        CGFloat height = 0;
        if (dynamic.text.value.length > 0) {//文字
            NSMutableAttributedString *content = [Utility emotionStrWithString:dynamic.text.value];
            [content addAttribute:NSFontAttributeName value:kContentFont range:NSMakeRange(0, content.length)];
            CGSize maxSize = CGSizeMake(kDynamicWidth, MAXFLOAT);
            CGSize attrStrSize = [content boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin context:nil].size;
        ...
        int imageCount = dynamic.imageCount.value;
        if (imageCount == 0) {
        ...
        return height;
    } else if (dynamic.type.value == 3){
        CGFloat height = 0;
        if (dynamic.text.value.length<= 0) {//不存在文字，肯定是纯视频
